...
from lecturer.forms import classForm, pubQnForm
...
def lect_publish(request, q_id, topic_id, period_id):
	...
	if request.method == "POST":
		form = pubQnForm(emp, topic.unit_id, period, request.POST)
		if form.is_valid():
			data = form.cleaned_data
			time = data.get('minutes_limit')
			curr_class = data.get('q_class')
			...
			if room is not None:
				...
				messages.error(request, 'Invalid room code', extra_tags='alert-warning')
				return redirect('lecturer:lect_publish', q_id=q_id, topic_id=topic_id, period_id=period)
		else:
			messages.error(request, 'Invalid form input', extra_tags='alert-warning')
			return redirect('lecturer:lect_publish', q_id=q_id, topic_id=topic_id, period_id=period)
	else:
		form = pubQnForm(emp, topic.unit_id, period)
		user_dict['form'] = form
...
def lect_q_stats(request, published_id):
	...
	if pub_qn is not None:
		...
		context = {
			...
			'correct_ans' : pub_qn.question.correct_answer,
